...
import { MoreHorizontal, X, Plus, Check } from "lucide-react";
...
export function WidgetStack({
  ...
  return (
    ...
    >
      {/* Top-gradient accent */}
      <div className={`absolute top-0 left-0 right-0 h-24 bg-gradient-to-b ${colorMap[stack.color] ?? colorMap.blue} pointer-events-none`} />

      {/* All-done glow overlay */}
      <AnimatePresence>
        {allDone && (
          <motion.div
            key="allDone"
        ...
        <div className="flex items-center gap-2 flex-1 min-w-0">
          {isEditingTitle ? (
            <input
              ...
              onBlur={() => { onRenameStack(stack.id, editTitle.trim() || stack.title); setIsEditingTitle(false); }}
          ...
          {totalCount > 0 && (
            <span className={`shrink-0 text-xs font-medium px-1.5 py-0.5 rounded-full ${allDone ? "bg-emerald-500/20 text-emerald-400" : "bg-white/10 text-zinc-400"}`}>
              {allDone ? <Check className="w-3 h-3 inline" /> : `${completedCount}/${totalCount}`}
            </span>
          )}
        </div>

        <div className="flex items-center gap-1 ml-2">
          <button onClick={() => onDeleteStack(stack.id)} className="text-zinc-600 hover:text-rose-400 transition-colors p-1 rounded opacity-0 group-hover:opacity-100">
      ...
      <div className="relative flex-1 px-2 pb-2">
        <AnimatePresence initial={false}>
          {stack.widgets.map((widget) => (
            <WidgetItem key={widget.id} widget={widget} onToggle={(wId) => onToggleWidget(stack.id, wId)} />
          ))}
        </AnimatePresence>
      </div>
      ...
      {/* Progress bar */}
      {totalCount > 0 && (
        <div className="relative h-1 bg-white/5 rounded-b-2xl overflow-hidden">
          <motion.div
